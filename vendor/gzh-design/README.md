...
改过**全局排版参数**（字号 / 行高 / 字间距 / 字体 / 段间距）时，再多跑一道产物页同步：

```bash
python3 scripts/_check_all.py         # 一条命令跑完全部验收
```

它内部依次跑：`_scan_typo.py`（组件库体检）→ `component_lint.py`（组件库反模式）→
`_stale_scan.py`（产物页旧值）→ `_verify_previews.py`（产物页逐项验收）→
`validate_gzh_html.py`（产物页微信合规）→ 再复跑一次 `_resync_previews.py` 确认幂等（应写入 0 个文件）。

单独重排产物页：

```bash
python3 scripts/_resync_previews.py   # 按组件库同一套参数重排（幂等，可反复跑）
```

> 组件库之外的产物页（样例页、区块库预览）不会自动跟着 `references/` 走，漏同步就会与组件库脱节。重排前备份在 `references/_bak_typo/gallery/`。
...
│   ├── _scan_typo.py           # 组件库排版参数体检（字号/行高/字距/字体/段间距）
│   ├── _stale_scan.py          # 产物页旧值体检
│   ├── _resync_previews.py     # 产物页重排（幂等，复用组件库同一套映射）
│   ├── _verify_previews.py     # 产物页验收（含文字逐字比对）
│   └── _check_all.py           # 一条命令跑完全部验收（组件库 + 产物页 + 微信合规）
├── assets/
...
| 主色 | 主题 | 适用 |
|---|---|---|
| ![](https://placehold.co/12/0F766E/0F766E.png) `#0F766E` | 摸鱼绿（默认） | 教程、测评、清单、工具盘点 |
| ![](https://placehold.co/12/B91C1C/B91C1C.png) `#B91C1C` | 红白色系 | 深度分析、观点、力量感话题 |
| ![](https://placehold.co/12/475569/475569.png) `#475569` | 石墨极简风 | 设计、科技评论、专业观点、高端品牌 |
| ![](https://placehold.co/12/556B4F/556B4F.png) `#556B4F` | 留白禅意风 | 禅意、极简生活、深度随笔 |
| ![](https://placehold.co/12/4F46E5/4F46E5.png) `#4F46E5` | 摸鱼票据风 | 工具对比、创意评测（票据视觉隐喻） |
