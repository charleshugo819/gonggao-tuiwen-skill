...
> - 正文字号 `14px`、行高 `2.5`、字间距 `2px`——**这是全主题统一值**（2026-09-18 起 6 套主题共用同一套排版参数，不要单独改本主题）
...
```html
<section style="max-width:677px;margin:0 auto;background:#ffffff;font-family:-apple-system,BlinkMacSystemFont,'PingFang SC','Hiragino Sans GB','Microsoft YaHei',sans-serif;color:#374151;line-height:2.5;letter-spacing:2px;">
...
<section style="background:#fffef8;border:2px solid #1a1a1a;box-shadow:4px 4px 0 #1a1a1a;margin-bottom:32px;">
  <section style="background:#4F46E5;padding:12px 20px;display:flex;justify-content:space-between;align-items:center;">
  ...
  <section style="display:flex;">
    <section style="flex:1;padding:24px 20px;border-right:2px dashed #C7D2FE;">
      <section style="font-size:24px;font-weight:900;color:#1a1a1a;letter-spacing:2px;margin-bottom:4px;text-shadow:0.5px 0 0 #1a1a1a;"><span leaf="">{{大标题}}</span></section>
      <section style="font-size:14px;color:#666;letter-spacing:2px;margin-bottom:20px;"><span leaf="">{{副标题}}</span></section>
      <section style="border-top:1px dashed #C7D2FE;margin-bottom:16px;"><span leaf=""><br></span></section>
      <section style="display:flex;align-items:center;gap:12px;margin-bottom:16px;">
        <!-- 有作者头像图时插入 <img> 到下面这个圆框里；无头像时整块删除该 section -->
        <section style="width:48px;height:48px;border-radius:50%;overflow:hidden;border:2px solid #4F46E5;flex-shrink:0;"><span leaf=""><br></span></section>
        <section>
      ...
      <section style="font-size:13px;color:#555;line-height:2.5;padding:12px;background:#EEF2FF;border:1px solid #C7D2FE;">
        <span leaf="">{{简介段落}}</span>
      </section>
      <section style="display:flex;gap:8px;margin-top:16px;">
        <section style="font-size:11px;color:#4F46E5;border:1px solid #4F46E5;padding:4px 10px;"><span leaf="">{{#标签1}}</span></section>
        <section style="font-size:11px;color:#4F46E5;border:1px solid #4F46E5;padding:4px 10px;"><span leaf="">{{#标签2}}</span></section>
        <section style="font-size:11px;color:#4F46E5;border:1px solid #4F46E5;padding:4px 10px;"><span leaf="">{{#标签3}}</span></section>
      </section>
    </section>
    <section style="width:48px;padding:14px 4px;display:flex;flex-direction:column;align-items:center;justify-content:space-between;background:#EEF2FF;">
      <section style="text-align:center;">
        <section style="font-size:11px;color:#999;letter-spacing:2px;"><span leaf="">NO.</span></section>
        <section style="font-size:18px;font-weight:900;color:#4F46E5;"><span leaf="">{{编号}}</span></section>
      </section>
      <section style="writing-mode:vertical-rl;font-size:11px;color:#888;letter-spacing:2px;"><span leaf="">{{竖排文字}}</span></section>
      <section style="text-align:center;">
        <section style="font-size:14px;font-weight:900;color:#4F46E5;"><span leaf="">{{等级}}</span></section>
      </section>
    </section>
  </section>
  <section style="display:flex;justify-content:space-between;align-items:center;padding:0 8px;">
    <section style="flex:1;border-top:2px dashed #C7D2FE;"><span leaf=""><br></span></section>
    <section style="padding:0 8px;font-size:11px;color:#C7D2FE;"><span leaf="">✂</span></section>
    <section style="flex:1;border-top:2px dashed #C7D2FE;"><span leaf=""><br></span></section>
  </section>
...
<section style="margin-bottom:32px;padding:0 20px;">
  <section style="display:flex;align-items:center;gap:12px;margin-bottom:24px;padding-bottom:12px;border-bottom:2px solid #1a1a1a;">
    <section style="background:#4F46E5;color:#fff;font-size:12px;font-weight:800;padding:6px 12px;letter-spacing:2px;"><span leaf="">{{编号}}</span></section>
    <section style="font-size:18px;font-weight:800;color:#1a1a1a;letter-spacing:2px;"><span leaf="">{{标题}}</span></section>
...
<section style="margin-bottom:32px;padding:0 20px;">
  <section style="display:flex;align-items:center;gap:8px;margin-bottom:16px;">
    <section style="width:4px;height:16px;background:#4F46E5;"><span leaf=""><br></span></section>
...
**用途**：普通正文，可内嵌组件 6 文字强调。字号行高为全主题统一值，不要单独改。

**可替换字段**：`{{正文内容}}`

```html
<section style="margin-bottom:32px;padding:0 20px;">
  <p style="font-size:14px;color:#555;line-height:2.5;margin-bottom:2.5em;text-align:justify;">
    <span leaf="">{{正文内容}}</span>
  </p>
</section>
```
...
```html
<span style="color:#4F46E5;font-weight:700;"><span leaf="">{{文字}}</span></span>
```

### 6b. 绿色高亮 —— 核心观点/关键数据

```html
<span style="background:linear-gradient(120deg,#C7D2FE 0%,rgba(167,243,208,0) 100%);padding:0 4px;font-weight:600;color:#111;"><span leaf="">{{文字}}</span></span>
```

### 6c. 绿色下划线 —— 重要短语（正文关键词的默认标记）

```html
<span style="border-bottom:2px solid #C7D2FE;font-weight:600;"><span leaf="">{{文字}}</span></span>
```
...
```html
<section style="margin-bottom:32px;padding:0 20px;">
  <section style="background:#EEF2FF;border-left:4px solid #4F46E5;padding:14px 16px;margin-bottom:0;">
    <p style="font-size:14px;color:#1a1a1a;font-weight:600;line-height:2.5;margin:0;">
      <span leaf="">{{前缀}}</span>
      <span style="color:#4F46E5;"><span leaf="">{{结论内容}}</span></span>
    </p>
  </section>
</section>
```
...
<section style="margin-bottom:32px;padding:0 20px;">
  <section style="background:#fffef8;border:1px solid #eee;margin-bottom:12px;">
    <section style="display:flex;align-items:stretch;">
      <section style="width:36px;background:#4F46E5;display:flex;align-items:center;justify-content:center;color:#fff;font-size:12px;font-weight:800;"><span leaf="">{{序号}}</span></section>
      <section style="flex:1;padding:12px 16px;font-size:13px;color:#555;line-height:2.5;border-left:1px dashed #C7D2FE;">
...
<section style="margin-bottom:32px;padding:0 20px;">
  <section style="background:#fffef8;border:2px solid #1a1a1a;box-shadow:3px 3px 0 #1a1a1a;padding:20px;margin-bottom:0;">
    <p style="font-size:15px;color:#1a1a1a;font-weight:700;line-height:2.5;margin:0 0 12px;text-align:center;">
      <span leaf="">{{金句前半}}</span>
      <span style="color:#4F46E5;font-size:24px;"><span leaf="">{{大数字}}</span></span>
      <span leaf="">{{金句后半}}</span>
    </p>
    <p style="font-size:14px;color:#555;line-height:2.5;margin:0;text-align:justify;">
      <span leaf="">{{补充说明}}</span>
    </p>
  </section>
</section>
...
<section style="padding:0 0 32px;">
  <section style="background:#fffef8;border:2px solid #1a1a1a;box-shadow:4px 4px 0 #1a1a1a;padding:24px 20px;text-align:center;">
    <p style="font-size:13px;font-weight:700;color:#1a1a1a;margin:0 0 20px;line-height:2.5;">
      <span leaf="">{{互动文案}}</span>
    </p>
    <section style="display:flex;justify-content:center;gap:24px;margin-bottom:16px;">
      ...
      <section style="text-align:center;color:#4F46E5;">
        <section style="width:40px;height:40px;display:flex;align-items:center;justify-content:center;margin:0 auto 6px;background:#EEF2FF;border:2px solid #4F46E5;">
        ...
        <span style="font-size:11px;font-weight:600;"><span leaf="">星标</span></span>
      </section>
    </section>
    <section style="border-top:1px dashed #ccc;padding-top:12px;">
      <p style="font-size:11px;color:#999;letter-spacing:2px;margin:0;">
...
本主题不设专属多行代码块组件；Markdown 的三反引号围栏代码块直接用 `common-components.md` 的 1a 深色代码块（默认）或 1b 浅色代码块，左竖条/强调色换成本主题主色 `#4F46E5`；行内代码用本主题组件 6d。
...
| ` ``` 多行代码块 ``` ` | 通用库 1a 深色（默认）/ 1b 浅色 | 左竖条换本主题主色 `#4F46E5` |
