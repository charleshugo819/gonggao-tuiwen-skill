...
版权归 **甲木 (Jiamu) × 摸鱼小李 (Moyu Xiaoli)** 所有，
采用 **GNU Affero General Public License v3.0（AGPL-3.0）** 许可，上游地址：
<https://github.com/isjiamu/gzh-design-skill>。

**本项目对该目录做了修改**（调整主题主色、删除标题内英文标签、统一排版参数与字体），修改日期 **2026-09-19**。

按 AGPL-3.0 的要求（对应上游 README 列明的四点）：

1. **必须署名** —— 保留版权与联名署名声明
2. **衍生品必须开源** —— 修改版本 / Fork / 二次分发须以 AGPL-3.0 公开发布并提供完整源码
3. **网络服务也要开源** —— 即使不分发代码、只是把修改版部署成服务给别人用，也须公开源码
4. **不允许闭源、专有化、仅付费分发**

本项目的落地要求：

- 完整许可证原文保留在 `vendor/gzh-design/LICENSE`，**不得删除**
- 原作者版权声明保留在 `vendor/gzh-design/` 内的各文件中，**不得删除**
- 本目录及其衍生作品整体继续以 AGPL-3.0 授权；**不得改用 MIT 或其他许可**

> ⚠️ **重要**：由于内嵌了 AGPL-3.0 组件，**整个仓库对外分发时须整体按 AGPL-3.0 处理**，
> 根目录的 `LICENSE` 不能只写 MIT。根 `LICENSE` 中的 MIT 仅覆盖自有部分。
>
> **公司内部同事之间使用**（不对外提供、不部署为对外服务）通常属于组织内部传播，
> 一般**不触发** AGPL 的源码提供义务；
> 但若**对外提供**（对外分发，或以网络服务形式让外部用户使用），
> 则须履行 AGPL-3.0 的源码公开义务。具体请以贵司法务意见为准。
